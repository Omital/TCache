...
namespace TCache.Tests
{
    public abstract class TCacheTestBase : AbpIntegratedTestBase<TCacheTestModule>
    {
        ...
        protected void UsingDbContext(Action<TCacheDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<TCacheDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<TCacheDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<TCacheDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<TCacheDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TCacheDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<TCacheDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<TCacheDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<TCacheDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<TCacheDbContext, Task<T>> func)
        {
            T result;
